...
function salvar() {
    ...
    if (numeroMac && modelo && fabricante && preço>0) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                 celular = new Celular(numeroMac, modelo,fabricante,preço);
                listaCelular.push(celular);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
               let celularAlterado = new Celular(numeroMac, modelo,fabricante, preço);
...
function mostrarDadoscelular(celular) {
    ...
    document.getElementById("inputPreço").value = celular.preço;
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    let nomeDoArquivoDestino = "./Celular.csv";  //define o modelo do arquivo csv
    let textoCSV = "";
    for (let i = 0; i < listaCelular.length; i++) {
